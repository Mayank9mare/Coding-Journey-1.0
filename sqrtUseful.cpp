...
struct sd{
    int n,bcnt,bsz;
    vector<int> a,bval,bl,br,bno,la,ld;
    ...
    void prop(int block_no){
        if(la[block_no]==0 && ld[block_no]==0)return;
        for(int i=bl[block_no];i<=br[block_no];i++){
            a[i]+=la[block_no]+(i-bl[block_no])*ld[block_no];
            bval[block_no]+=la[block_no]+(i-bl[block_no])*ld[block_no];
