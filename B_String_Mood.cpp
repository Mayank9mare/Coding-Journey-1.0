...
struct Matrix
{
    ...
    vector<vector<int>> cofactor(int p,int q){
        vector<vector<int>> temp(n_rows,vector<int>(n_cols));
        int n=n_rows;
        int i = 0, j = 0;
    ...
    for (int f = 0; f < n; f++)
    {
       ...
       vector<vector<int>> temp= cofactor(0, f);
...
int solve(){
    ll n;
    cin>>n;
     Matrix m=Matrix({{19,7},{6,20}});

     Matrix m2=fast_exponentiation(m,n);
     cout<<m2.mat[0][0]<<endl;
    return 0;
